...
Voice transcription: `ENABLE_VOICE_MESSAGES` (default true), `VOICE_PROVIDER` (`mistral`|`openai`|`local`|`taps`, default `mistral`), `MISTRAL_API_KEY`, `OPENAI_API_KEY`, `VOICE_TRANSCRIPTION_MODEL`. For local provider: `WHISPER_CPP_BINARY_PATH`, `WHISPER_CPP_MODEL_PATH` (requires ffmpeg + whisper.cpp installed). For taps provider: `TAPS_BINARY_PATH` (default `~/Projects/taps/transcribe.sh`), `TAPS_MODEL`, `TAPS_LANGUAGE` (default `ru`), `TAPS_EXTRA_ARGS`, `TAPS_TIMEOUT_SECONDS` — each env var overrides the matching field in TAPS `config.toml`; unset values fall back to TAPS defaults. Provider implementation is in `src/bot/features/voice_handler.py`.
